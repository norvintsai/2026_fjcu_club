...
export function OtpInput({
  ...
  const { chars, focusedIndex, getCellProps, focusAt, clear } = useOtpInput({
    length, mode, defaultValue, disabled, onChange, onComplete,
  });
  ...
  const messageTone = error ? "text-[#ff3366]" : success ? "text-[#00ff88]" : "text-[#4a4a6a]";
  ...
  return (
    <div className={`inline-flex flex-col ${className}`}>
        ...
        {Array.from({ length }, (_, i) => {
          ...
          const cellBorder = error
            ...
            : success
              ? "border-[#00ff88]"
              : active
                ? "border-[#ff3366] shadow-[0_0_10px_rgba(255,51,102,.35)]"
                : char
                  ? "border-[#ff336640]"
                  : "border-[#1e1e32]";
          ...
          return (
            <div key={i} className={`relative h-12 w-10 ${gap ? "ml-3" : ""}`}>
              <input
                ...
                className={`h-12 w-10 rounded-[6px] border-2 text-center text-[15px] text-transparent caret-transparent outline-none transition-all duration-150 selection:bg-transparent focus-visible:outline-none disabled:opacity-40 bg-[#0a0a12] ${cellBorder}`}
              />
              ...
              <span aria-hidden className="pointer-events-none absolute inset-0 grid place-items-center">
                <AnimatePresence initial={false} mode="popLayout">
                  {char ? (
                    <motion.span
                      ...
                      transition={enter}
                      className="col-start-1 row-start-1 font-mono text-[15px] tabular-nums"
                      style={{ color: error ? "#ff3366" : success ? "#00ff88" : "#ff9999" }}
                    >
                      {mask ? "•" : char}
                    </motion.span>
                  ) : null}
                ...
                {active && !char && !disabled ? (
                  <motion.span
                    className="col-start-1 row-start-1 block h-[17px] w-[1.5px] rounded-[1px] bg-[#ff3366]"
                    initial={{ opacity: 1 }}
      ...
      {hasStatus && (
        <>
          <div aria-hidden className="mt-2 grid h-4 text-[11px] leading-[16px] font-mono tracking-wider">
          ...
          <span id={statusId} role="status" className="sr-only">{message}</span>
        </>
      )}
    </div>
  );
